...
def main():
    ...
    #Else if input file is a csv file
        ...
        #Create edgelist with full names
        #nu.create_edgelist_full_names(input_file,
         #                             os.path.join("..", "data", "edgelists", "full_names_" + args["output_edgelist"]),
          #                            label,
           #                           batch = batch_size)
        
        #Create edgelist with first names
        nu.create_edgelist(input_file,
                           os.path.join("..", "data", "edgelists", "house_" + args["output_edgelist"]),
                           label,
                           name_pos = "full_name",
                           batch = batch_size)
        ...
        #Create edgelist with last names
        #nu.create_edgelist(input_file,
        #                   os.path.join("..", "data", "edgelists", "house_" + args["output_edgelist"]),
        #                   label,
        #                   name_pos = -1,
    ...
    print("Creating edgelist ...")
    #Create edgelist
    nu.create_edgelist(output_csv, output_edgelist, label, batch = batch_size)
